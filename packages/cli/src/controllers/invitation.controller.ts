...
/**
 * Controller for handling user invitation flows.
 */
...
export class InvitationController {
	...
	async inviteUser(
		...
		const isWithinUsersLimit =
			process.env.N8N_SKIP_LICENSE_CHECK === 'true' || this.license.isWithinUsersLimit();
		...
		// Trigger external hooks for invited users
	...
	/**
	 * Accept an invitation: set password, name, and issue auth cookie.
	 */
	...
	async acceptInvitation(
		...
		if (users.length !== 2) {
			this.logger.debug('Invalid inviter or invitee ID on accept invitation', {
				inviterId,
				inviteeId,
			});
		...
		const invitee = users.find((u) => u.id === inviteeId) as User;
		if (invitee.password) {
			this.logger.debug('Invitation accept failed: invite already accepted', { inviteeId });
			throw new BadRequestError('This invite has been accepted already');
		}

		// Populate the invitee record
		...
		// Issue auth cookie and emit signup event
		...
		// Return public user with scopes and analytics
